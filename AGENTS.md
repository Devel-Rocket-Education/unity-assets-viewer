...
> 이 파일은 **`CLAUDE.md`와 완전히 동일한 내용**이다. 한쪽을 고치면 다른 쪽도 반드시 같이 갱신한다.
> (`AGENTS.md` = Codex·Antigravity 등 `agents.md` 규약을 읽는 에이전트, `CLAUDE.md` = Claude Code. 파이프라인은 순수 Node.js라 특정 도구에 종속되지 않는다.)
